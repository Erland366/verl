...
If you only need the core native ``verl`` install on ROCm, without the ROCm ``vLLM`` or ``SGLang`` source-build path documented on this page, use ``bash scripts/install_verl_amd_conda.sh`` from the repo root instead. That script installs the validated core ``verl`` stack into the ``verlrl`` conda environment, includes ``trl`` for PPO critic/value-head initialization, adds the extra runtime packages used by the native smoke path, applies the local Triton compatibility shims required by the PyTorch 2.7 ROCm stack on this host, and runs a smoke test that defaults to eager attention on ROCm. Use the Docker workflow on this page when you need the AMD-specific inference stack as well.

For the validated MI210 ``gfx90a`` native smoke path, we currently use a local ROCm ``vLLM`` 0.10 source checkout under ``/tmp/vllm-v0.10.0`` together with the ROCm ``amdsmi`` Python bindings shipped under ``/opt/rocm-6.3.3/share/amd_smi``. The AMD smoke launcher wires both paths into ``PYTHONPATH`` automatically when ``ROLLOUT_NAME=vllm``. On this native path, ``verl`` accepts ``vLLM`` version discovery from ``vllm.__version__`` so the source checkout does not need to be installed as a wheel first. When that ROCm ``vLLM`` build does not expose a hardware UUID lookup API yet, ``verl`` falls back to the visible accelerator id for rollout IPC socket naming. The MI210 smoke launcher defaults to ``ADV_ESTIMATOR=grpo`` so the GSM8K smoke path avoids the separate PPO critic model; set ``ADV_ESTIMATOR=gae`` only when debugging the critic path directly. It also defaults ``vLLM`` to ``compilation_config.mode=0``, ``compilation_config.backend=eager``, and ``compilation_config.cudagraph_mode=NONE``, disables rollout sleep mode with ``free_cache_engine=False`` and ``enable_sleep_mode=False`` because ``vLLM`` 0.10 rejects sleep mode on ROCm, disables FSDP ``use_torch_compile`` for the smoke-path ``actor``, ``ref``, and ``critic`` workers including their nested FSDP engine configs so native ROCm startup stays off the current TorchInductor and Triton path, keeps actor and critic ``use_remove_padding`` enabled while falling back to Hugging Face padding helpers when the native ``verlrl`` environment does not provide the CUDA ``flash_attn.bert_padding`` module, and exports ``HSA_FORCE_FINE_GRAIN_PCIE=1``, ``RCCL_MSCCL_ENABLE=0``, and ``RCCL_MSCCLPP_ENABLE=0`` for a more conservative MI210 PCIe RCCL setup. The validated 4-GPU native GRPO smoke path currently uses ``tensor_model_parallel_size=1``, ``data_parallel_size=4``, and ``data_parallel_external_lb=True`` so each GPU hosts one external-DP ``vLLM`` server. On this path, ``verl`` normalizes each ``vLLM`` server actor to a single visible-device environment variable before engine subprocess spawn and launches all external-DP ranks in non-headless mode so ROCm Ray worker imports and external-DP engine startup stay compatible. The validated native installer also patches Triton for the current PyTorch 2.7 ROCm ABI mismatches, including ``triton_key``, missing ``CompiledKernel`` launch hooks, and missing compiled-kernel ``cluster_dims`` compatibility fields. It keeps ``RAY_EXPERIMENTAL_NOSET_HIP_VISIBLE_DEVICES=1`` by default so zero-GPU Ray actors do not lose HIP visibility before import-time ROCm platform detection.
